...
#
#	Copyright (c) 2003-2011 by GWENDESIGN
#	All rights reserved.
#
#	----------------------------------------------------------------------
#	IR-Blaster
...
#			- Sending an IR command if SB2/3, Transporter or SB Touch is powered on / off
#			- Relaying volume up / down to a receiver / amplifier (via ir)
#			- Multiple IR commands can be sent for each action
#
#			- IR Repeater (only SB2/3 and Transporter): Relaying any IR commands to other equipment
#
#			- IR Learning (only SB2/3 and Transporter): Learn ir commands
#
...
#			   phone/IR jack of your SB2/SB3, Transporter or
#			   SB Touch
#
#	----------------------------------------------------------------------
#	History:
...
sub irblasterSendCommandCLI {
	...
	$log->debug( "*** IR-Blaster CLI Device: " . $device . "\n");
	$log->debug( "*** IR-Blaster CLI Button: " . $button . "\n");
...
sub reportOnOurPlayers() {
        ...
        foreach my $client (Slim::Player::Client::clients()) {
                if( $classPlugin->checkVolumeCommandAssigned( $client)) {
                        ...
                        $log->debug( "*** IR-Blaster: Note that " . $client->name() . " uses us for external volume control");
...
#### fm
# Fab4 only supports NEC and RC5 formats (ITT, RC6 and RAW are not supported in firmware)
# ITT format is experimental and requires firmware changes to Fab4 MCU
if( $client->model() eq 'fab4') {
			# IR string format for fab4 (16 bytes total)

			# Code:		8 bytes = 64 bits (right aligned)
			# Bitcount:	1 byte	Relevant bits in code
			# Header pre:	1 byte	(divide by 100 to fit in byte)	not sent if 0
			# Header post:	1 byte	(divide by 100 to fit in byte)	not sent if 0
			# Bit0 pre:	1 byte	(divide by 25 to fit in byte)	if most significant bit set -> mark, else space
			# Bit0 post:	1 byte	(divide by 25 to fit in byte)	if most significant bit set -> mark, else space
			# Bit1 pre:	1 byte	(divide by 25 to fit in byte)	if most significant bit set -> mark, else space
			# Bit1 post:	1 byte	(divide by 25 to fit in byte)	if most significant bit set -> mark, else space
			# Trail:	1 byte	(divide by 25 to fit in byte)	not sent if 0
			...
			if( $IR_FLAGS =~ m/SPACE_ENC/) {
				my $irnumbits = $IR_PRE_DATA_BITS + $IR_BITS + $IR_POST_DATA_BITS;

# Fix for total command length bigger than 32 bits - standard Perl variables are only 32 bits!!! - we support a total length of 64 bits.
# One single section (pre, code, post) still cannot be more than 32 bits.
#
#				my $fullircmd = ( ( ( $IR_PRE_DATA << $IR_BITS) | $code) << $IR_POST_DATA_BITS) | $IR_POST_DATA;
#				$fullircmd = sprintf("%016X", $fullircmd);
				...
				$irCmdLow32 |= $IR_POST_DATA;
				$numBitsUsed += $IR_POST_DATA_BITS;
				$tempCode = $code;
				$irCmdLow32 |= $tempCode << $numBitsUsed;
				$numBitsUsed += $IR_BITS;
				if( $numBitsUsed > 32) {
					$tempCode = $code;
					$irCmdHigh32 |= $tempCode >> ( 32 - $IR_POST_DATA_BITS);
					$tempCode = $IR_PRE_DATA;
					$irCmdHigh32 |= $tempCode << ( $numBitsUsed - 32);
				} else {
					$tempCode = $IR_PRE_DATA;
					$irCmdLow32 |= $tempCode << $numBitsUsed;
					$numBitsUsed += $IR_PRE_DATA_BITS;
					if( $numBitsUsed > 32) {
						$tempCode = $IR_PRE_DATA;
						$irCmdHigh32 |= $tempCode >> ( 32 - $IR_POST_DATA_BITS - $IR_BITS);
					}
				}
				...
				$irstr .= " 0x" . substr( $fullircmd, 8, 2);
				$irstr .= " 0x" . substr( $fullircmd, 10, 2);
				...
				if( ( $IR_HEADER_H != 0) && ( $IR_HEADER_L != 0)) {
					...
					if( $iRepeatCount == 0) {
						$irstr .= sprintf( " 0x%02X", int( $IR_HEADER_H / 100));
						$irstr .= sprintf( " 0x%02X", int( $IR_HEADER_L / 100));
				...
				# ZERO (only sent if not 0)
				if( ( $IR_ZERO_H != 0) && ( $IR_ZERO_L != 0)) {
					$irstr .= sprintf( " 0x%02X", int( $IR_ZERO_H / 25) + 0x80);
					$irstr .= sprintf( " 0x%02X", int( $IR_ZERO_L / 25));
				} else {
					$irstr .= " 0x00";
					$irstr .= " 0x00";
				}

				# ONE (only sent if not 0)
				if( ( $IR_ONE_H != 0) && ( $IR_ONE_L != 0)) {
					$irstr .= sprintf( " 0x%02X", int( $IR_ONE_H / 25) + 0x80);
					$irstr .= sprintf( " 0x%02X", int( $IR_ONE_L / 25));
				} else {
					$irstr .= " 0x00";
					$irstr .= " 0x00";
				}
				...
				if( ( $IR_PTRAIL != 0) && ( $IR_GAP != 0)) {
					$irstr .= sprintf( " 0x%02X", int( $IR_PTRAIL / 25));
				} else {
					$irstr .= " 0x00";
				}
			...
			# Setting the first parameter (high-time) to 1 forces
			#  the firmware to use ITT mode which
			#  consists of 20us pulses with different low-times and no carrier

			} elsif( $IR_FLAGS =~ m/ITT/) {

				my $fullircmd = sprintf("%016X", $code);
				...
				$irstr .= sprintf( " 0x%02X", $IR_BITS);

				# Header (only sent if not 0)
				if( ( $IR_HEADER_H != 0) && ( $IR_HEADER_L != 0)) {
					$irstr .= sprintf( " 0x%02X", int( $IR_HEADER_H));
					$irstr .= sprintf( " 0x%02X", int( $IR_HEADER_L));
				} else {
					$irstr .= " 0x00";
					$irstr .= " 0x00";
				}
				...
				# ZERO (only sent if not 0)
				if( ( $IR_ZERO_H != 0) && ( $IR_ZERO_L != 0)) {
					$irstr .= sprintf( " 0x%02X", int( $IR_ZERO_H));
					$irstr .= sprintf( " 0x%02X", int( $IR_ZERO_L));
				} else {
					$irstr .= " 0x00";
					$irstr .= " 0x00";
				}
				...
				# ONE (only sent if not 0)
				if( ( $IR_ONE_H != 0) && ( $IR_ONE_L != 0)) {
					$irstr .= sprintf( " 0x%02X", int( $IR_ONE_H));
					$irstr .= sprintf( " 0x%02X", int( $IR_ONE_L));
				} else {
					$irstr .= " 0x00";
					$irstr .= " 0x00";
				}
				...
				# Ptrail (only sent if not 0)
				if( ( $IR_PTRAIL != 0) && ( $IR_GAP != 0)) {
					$irstr .= sprintf( " 0x%02X", int( $IR_PTRAIL));
				} else {
					$irstr .= " 0x00";
				}
...
#				$fullircmd = sprintf("%016X", $fullircmd);
				...
				if( $IR_PLEAD != 0) {
					$irstr .= sprintf( " 0x%02X", int( $IR_PLEAD / 100));
					$irstr .= sprintf( " 0x%02X", 0);
				} else {
					$irstr .= " 0x00";
					$irstr .= " 0x00";
				}
				...
				# ONE (only sent if not 0)
				if( ( $IR_ONE_H != 0) && ( $IR_ONE_L != 0)) {
					$irstr .= sprintf( " 0x%02X", int( $IR_ONE_H / 25));
					$irstr .= sprintf( " 0x%02X", int( $IR_ONE_L / 25) + 0x80);
				} else {
					$irstr .= " 0x00";
					$irstr .= " 0x00";
				}
			...
			$log->debug( "*** IR-Blaster: irstr: " . $irstr . "\n");


			Slim::Utils::Timers::setTimer( $client,
				Time::HiRes::time() + $nextTime,
				\&IRBlastSendCallbackFab4,
				(\$irstr));
			
##			$nextTime += 0.05;
			$nextTime += 0.1;

# SB2/3 and Transporter
# Support NEC, ITT, RC5, RC6 and RAW formats
} else {
####
...
# Block sending while still busy
		...
		$log->debug( "*** IR-Blaster: RAW: num of codes: " . $numCodes . "\n");
...
# Block sending while still busy
				...
				$log->debug( "*** IR-Blaster: RAW: mark: " . $arrCodes[$i] . " space: " . $arrCodes[$i+1] . "\n");
...
# Block sending while still busy
				...
				$log->debug( "*** IR-Blaster: RAW: mark: " . $arrCodes[$i] . " gap: " . $IR_GAP . "\n");
...
# IR Blaster:
# ----------------------------------------------------------------------------
sub IRBlastSendCallbackFab4 {
	my $client = shift;
	my $irstrref = shift;

	$log->debug( "*** IR-Blaster: " . $$irstrref . "\n");
	...
	$client->sendFrame( 'blst', $irstrref);
}
...
sub commandCallback {
	...
	my $shell_output = system("/var/lib/squeezeboxserver/cache/InstalledPlugins/Plugins/IRBlaster/custom-run-on-action.sh", $client->name(), $iPowerOld, $iPower, $request->{'_request'}[0]);
